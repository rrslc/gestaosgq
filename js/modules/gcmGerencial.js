/**
 * @fileoverview GCM — Gerencial: painel panorâmico e acompanhamento de ações.
 */
...
function renderPainel() {
  ...
  const kpis = {
    total:        all.length,
    aberta:       all.filter(r => r.status === 'Aberta').length,
    analise:      all.filter(r => r.status === 'Em Análise').length,
    aprovada:     all.filter(r => r.status === 'Aprovada').length,
    implantacao:  all.filter(r => r.status === 'Em Implantação').length,
    concluida:    all.filter(r => r.status === 'Concluída').length,
    rejeitada:    all.filter(r => r.status === 'Rejeitada').length,
  };

  const PIPELINE = [
    { key: 'Aberta',         color: 'var(--red)',    n: kpis.aberta },
    { key: 'Em Análise',     color: 'var(--purple)', n: kpis.analise },
    { key: 'Aprovada',       color: 'var(--blue)',   n: kpis.aprovada },
    { key: 'Em Implantação', color: 'var(--amber)',  n: kpis.implantacao },
    { key: 'Concluída',      color: 'var(--green)',  n: kpis.concluida },
  ];
  ...
  function diasAberto(r) {
    ...
    const aberto = !['Concluída', 'Rejeitada', 'Cancelada'].includes(r.status);
    const cor = aberto && r.prazoImplementacao && new Date(r.prazoImplementacao + 'T00:00:00') < hoje ? 'var(--red)' : 'inherit';
  ...
  const tableHtml = all.length ? `
    <div class="table-wrap">
      <table>
        <thead><tr>
          <th>Número</th><th>Categoria</th><th>Descrição</th><th>Solicitante</th>
          <th>Impacto</th><th>Data</th><th>T. Aberto</th><th>Status</th><th>Eficácia</th>
        </tr></thead>
        <tbody>
          ${all.map(r => `<tr>
  ...
  ` : emptyState('Nenhuma mudança registrada.');
  ...
  return `
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">
      ...
      ${kpiCard(kpis.aberta + kpis.analise + kpis.aprovada + kpis.implantacao, 'Em Aberto', 'var(--amber)')}
      ${kpiCard(kpis.concluida,   'Concluídas',        'var(--green)')}
      ${kpiCard(kpis.rejeitada,   'Rejeitadas',        'var(--muted)')}
    </div>
    <div class="card">
      <div style="font-weight:600;margin-bottom:12px;font-size:0.9rem">Todas as Mudanças</div>
      ${tableHtml}
    </div>
  `;
}
...
function renderFiltrosBar(acoes) {
  ...
  const gcmNums      = [...new Set(acoes.map(a => a.gcmNumero).filter(Boolean))].sort();
  const hasFilter    = acaoFiltros.busca || acaoFiltros.status || acaoFiltros.responsavel || acaoFiltros.gcm || acaoFiltros.etapa;

  return `
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:12px">
      <input id="acao-busca" type="text" placeholder="Buscar ação ou Nº CM…" style="${INP_STYLE}" value="${acaoFiltros.busca}">
      ...
      <select id="acao-gcm" style="${SEL_STYLE}">
        <option value="">Todas as CMs</option>
        ${gcmNums.map(n => `<option value="${n}" ${acaoFiltros.gcm === n ? 'selected' : ''}>${n}</option>`).join('')}
      </select>
...
function applyFiltros(acoes) {
  ...
  return acoes.filter(a => {
    if (acaoFiltros.busca) {
      ...
      if (!a.acao.toLowerCase().includes(q) && !(a.gcmNumero || '').toLowerCase().includes(q)) return false;
    }
    ...
    if (acaoFiltros.gcm         && a.gcmNumero   !== acaoFiltros.gcm)         return false;
    return true;
  });
}
...
function renderAcoesTableBody(allAcoes) {
  ...
  const tableHtml = filtered.length ? `
    <div class="table-wrap">
      <table>
        <thead><tr>
          <th>Nº CM</th><th>Etapa</th><th>Ação</th><th>Responsável</th><th>Prazo</th><th>Status</th><th>Conclusão</th><th>Ações</th>
        </tr></thead>
        <tbody>
          ${filtered.map(a => {
            ...
            const etapaBadge = a.etapa
              ...
              : '—';
            return `<tr>
              <td><strong>${a.gcmNumero}</strong></td>
              <td>${etapaBadge}</td>
...
function renderPrazos(gcms) {
  const hoje    = new Date(); hoje.setHours(0, 0, 0, 0);
  const CLOSED  = ['Concluída', 'Rejeitada', 'Cancelada'];
  const open    = gcms.filter(r => !CLOSED.includes(r.status));

  if (!open.length) return emptyState('Nenhuma CM em aberto.');

  return `
    <div class="table-wrap">
      <table>
        <thead><tr>
          <th>Nº CM</th><th>Categoria</th><th>Status</th><th>Data Abertura</th>
          <th>Prazo de Implementação</th><th>Situação</th>
        </tr></thead>
        <tbody>
          ${open.map(r => {
            const prazo = r.prazoImplementacao;
...
function renderAcoesPrazos() {
  const allAcoes = db.get('gcmAcoes');
  ...
  return `
    ...
    <div class="card">
      <div style="font-weight:600;margin-bottom:12px;font-size:0.9rem">Prazos de Implementação</div>
      ${renderPrazos(gcms)}
    </div>
  `;
}
...
function fieldsAcao(gcms) {
  const equipe = db.get('equipe').map(m => m.nome);
  return [
    { id: 'gcmRef',        label: 'CM',              type: 'select',   required: true,  span: 2, options: gcms.map(c => `${c.numero} — ${c.descricao.slice(0, 40)}`) },
...
let acaoFiltros = { busca: '', status: '', responsavel: '', gcm: '', etapa: '' };
let _searchTimer;
  ...
  init(container) {
    container.addEventListener('click', e => {
      ...
      if (action === 'limpar-filtros') {
        acaoFiltros = { busca: '', status: '', responsavel: '', gcm: '', etapa: '' };
      ...
      if (action === 'new-acao') {
        const gcms = db.get('gcm');
        openModal({
          title: 'Nova Ação de CM',
          fields: fieldsAcao(gcms),
          data: { status: 'Pendente' },
          onSave: data => {
            const gcm = gcms.find(c => c.numero === data.gcmRef?.split(' — ')[0]) ?? gcms[0];
            db.add('gcmAcoes', {
              gcmId: gcm?.id ?? 0, gcmNumero: gcm?.numero ?? '',
      ...
      if (action === 'edit-acao') {
        const record = db.getById('gcmAcoes', numId);
        if (!record) return;
        const gcms   = db.get('gcm');
        const fields = fieldsAcao(gcms).filter(f => f.id !== 'gcmRef');
        openModal({
          title: 'Editar Ação',
          fields,
          data: record,
          onSave: data => {
            db.update('gcmAcoes', numId, data);
    ...
    container.addEventListener('change', e => {
      ...
      else if (e.target.id === 'acao-gcm')         acaoFiltros.gcm         = e.target.value;
      else return;
    ...
    container.addEventListener('input', e => {
      ...
      _searchTimer = setTimeout(() => {
        ...
        if (wrap) wrap.innerHTML = renderAcoesTableBody(db.get('gcmAcoes'));
      }, 280);
    });
  },
};
